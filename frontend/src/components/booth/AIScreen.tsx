...
interface AIStyle { key: string; name: string; emoji: string; }
...
export function AIScreen() {
  ...
  // Send photoId to backend — backend fetches from R2 server-side (no CORS issue)
  async function callAI(endpoint: string, body: Record<string, string>) {
    const form = new FormData();
    Object.entries(body).forEach(([k, v]) => form.append(k, v));
    const res = await fetch(`${API_BASE}/api/ai/${endpoint}`, { method: 'POST', body: form });
    if (!res.ok) {
  ...
  async function handleGenerate(styleKey: string) {
    ...
    try {
      ...
      toast.error(msg.includes('MODEL_LOADING') ? 'AI warming up — try again in 30 seconds' : msg);
      setAIProgress('');
    } finally {
  ...
  async function handleSurprise() {
    ...
    try {
      ...
      const result = await callAI('surprise', { eventId: event.id, photoId: currentPhoto.id });
      setGeneratedPhoto(result.ai.url);
  ...
  return (
    <div className="w-full h-full flex flex-col bg-[#0a0a0f]">
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
        <button onClick={() => setScreen('preview')} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors" disabled={aiGenerating}>
      ...
      <div className="flex-1 flex gap-3 p-4 overflow-hidden min-h-0">
        <div className="flex-1 relative rounded-2xl overflow-hidden bg-white/5">
          <div className="absolute top-3 left-3 bg-black/60 rounded-lg px-2 py-1 text-xs text-white/70">Original</div>
        ...
        <div className="flex-1 relative rounded-2xl overflow-hidden bg-white/5 border border-purple-500/30">
          <div className="absolute top-3 left-3 bg-purple-600/80 rounded-lg px-2 py-1 text-xs text-white z-10">AI Version</div>
          {aiGenerating && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 z-20">
              ...
              <p className="text-white/40 text-xs mt-2">This may take 30-60 seconds</p>
            </div>
          )}
          {generatedPhoto ? (
            // eslint-disable-next-line @next/next/no-img-element
      ...
      <div className="px-4 pb-4 space-y-3">
        <motion.button whileTap={{ scale: 0.95 }} onClick={handleSurprise} disabled={aiGenerating}
          className="w-full py-4 rounded-2xl font-bold text-white flex items-center justify-center gap-3 disabled:opacity-50 transition-all btn-touch glow-gold"
          ...
          {styles.map((style) => (
            <motion.button key={style.key} whileTap={{ scale: 0.93 }}
              ...
              className={`py-3 px-2 rounded-xl text-center transition-all btn-touch disabled:opacity-50 border
                ${selectedStyle === style.key ? 'bg-purple-600/40 border-purple-500 text-white' : 'bg-white/5 border-white/10 text-white/70 hover:border-white/30'}`}>
