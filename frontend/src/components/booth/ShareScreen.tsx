...
export function ShareScreen() {
  ...
  // Build gallery URL from current domain — guarantees correct URL regardless of DB value
  // Format: https://current-domain/gallery/event-slug?photo=photo-id
  const galleryBase = typeof window !== 'undefined' ? window.location.origin : '';
  const galleryUrl = event?.slug && photo.id
    ? `${galleryBase}/gallery/${event.slug}?photo=${photo.id}`
    : photo.galleryUrl || photo.url;
  const photoUrl = galleryUrl;
