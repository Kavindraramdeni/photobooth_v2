...
export function IdleScreen() {
  ...
  useEffect(() => {
    if (videoRef.current && idleMediaUrl && isVideo) {
      videoRef.current.play().catch(() => setMediaError(true));
    }
  }, [idleMediaUrl, isVideo]);
  ...
  if (idleMediaUrl && !mediaError) {
    return (
      ...
      >
        {isVideo ? (
          <video ref={videoRef} src={idleMediaUrl} className="absolute inset-0 w-full h-full object-cover"
            autoPlay loop muted playsInline onError={() => setMediaError(true)} />
        ) : (
        ...
        {modes.length > 1 && (
          <div className="absolute bottom-6 right-20 flex flex-col gap-2 z-20" onClick={(e) => e.stopPropagation()}>
            {modes.slice(1).map((m) => (
  ...
  return (
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}
        className="z-10 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <motion.button
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {modes.slice(1).map((m) => (
              <motion.button key={m.mode} whileTap={{ scale: 0.93 }} onClick={() => handleStart(m.mode)}
                className="py-5 rounded-2xl bg-white/10 border border-white/20 text-white font-semibold flex flex-col items-center gap-2 hover:bg-white/15 transition-all">
